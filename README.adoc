...
The https://hub.docker.com/r/grafana/otel-lgtm[Grafana OTel LGTM Docker image^] provides an all-in-one setup with an OpenTelemetry Collector that receives telemetry data and forwards it to Tempo for traces, Prometheus for metrics, and Loki for logs. Grafana can be used to visualize the collected data.
...
When the container is up and running, you can access the Grafana dashboard at the http://localhost:3000 URL.
